...
interface FavouriteCitiesDao {
    ...
    @Query("SELECT EXISTS (SELECT * FROM favourite_cities WHERE id=:citiId LIMIT 1)")
    fun observeIsFavourite(citiId: Int): Flow<Boolean>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun addToFavourite(cityDbModel: CityDbModel)

    @Query("DELETE FROM favourite_cities WHERE id=:citiId")
    suspend fun removeFromFavourite(citiId: Int)
}